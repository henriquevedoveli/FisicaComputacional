...
rho = 28.0  # numero de Rayleigh em 28 exibe um corpotamento caotico
sigma = 10.0
beta = 8.0 / 3.0
####################

vecT = np.arange(t0, tmax, dt)
n = len(vecT)


###################################
#  EDOs para o atrator de Lorenz  #
###################################
...
for i in range(n - 1):
    evol[i + 1, :] = RK4(vecT[i], evol[i, 0], evol[i, 1], evol[i, 2], dx, dy, dz, dt)

fig = plt.figure(figsize=(12, 6), dpi=100)  # (12, 6); (12, 7); (14, 9)
ax = fig.gca(projection='3d')
fig.canvas.set_window_title('Atractor de Lorenz')
...
def update(i):
    ...
    # ax.autoscale_view()
    fig.tight_layout()
    ax.plot(evol[:i, 0], evol[:i, 1], evol[:i, 2], color='white', lw=0.7, label='Atrator de Lorenz: t = ' + str(i))
...
ani = animation.FuncAnimation(fig, update, np.arange(5001), interval=10, repeat=False)
# ani.save('Lorenz.gif', writer = PillowWriter(fps = 24))
